    ...
    def _analyze_and_report(self, pattern, case_sensitive, name, directive):
        ...
        for candidate_match in regexp.generate('`', anchored=True, max_repeat=5):
            ...
            if self.allowed_domains:
                if not any(self.same_origin(base_hostname_filled, d) for d in self.allowed_domains):
                    self.insecure_set.add(base_mutant_raw_filled)
                    continue
                if not any(self.same_origin(suffix_hostname_filled, d) for d in self.allowed_domains):
                    self.insecure_set.add(suffix_mutant_raw_filled)
                    continue
                if not any(self.same_origin(prefix_hostname_filled, d) for d in self.allowed_domains):
                    self.insecure_set.add(prefix_mutant_raw_filled)
                    continue
